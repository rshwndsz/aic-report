...
% \begin{enumerate}
%  \item The output waveform for the given input was obtained with a phase of 92.13^{\circ} , an error of 2.4\%.
%  \item The ouput voltage saturates at V_{sat} at low frequencies, when the resistor R_{F} is removed.
%  \item A triangular waveform is observed when a square wave is given as input. This waveform gets clipped at low frequencies.
% \end{enumerate}
  
  It can therefore be seen that a practical integrator, which has a resistor $R_{F}$ in parallel with capacitor C, overcomes the drawbacks of an ideal integrator. 
